  ...
  return (
    <>
        <div className="flex flex-col justify-center items-center mb-60">
            <div className="border border-[var(--primary)] text-[var(--primary)] font-semibold rounded-xl px-6 py-2 w-fit uppercase">READ</div>
            <div className='text-[5rem] font-semibold max-w-7xl leading-[5rem] py-6 text-center'>Insights & AI Innovations</div>
...
function InsightsCard({image, tag, heading, subheading}: {
    ...
    return (
        <>
            <div className="flex flex-col gap-4 bg-neutral-900 rounded-2xl px-4 py-8 text-gray-400">
                <div className="flex justify-center">
                    <Image src={image} alt="blob1" height={100} width={100} className="h-[12rem] w-[12rem]" />
                </div>
                <div className="font-medium text-[1.2rem]">{tag}</div>
                <div className="text-[2rem] font-semibold text-white max-w-sm">{heading}</div>
                <div className="text-[1.2rem] font-medium max-w-sm">{subheading}</div>
            </div>
        </>
    )
}
